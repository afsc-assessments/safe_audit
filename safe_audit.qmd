...
This view groups stocks by FMP first, then by the most relevant sub-area within that FMP. GOA entries are separated into `GOA` versus `Both` where needed, while BSAI entries are separated into `EBS`, `AI`, or `Both`.
...
If you need the row-level detail, the table below expands the same collisions back out to one row per filename-year combination.

```{r}
safe_summaries$filename_collisions |>
  arrange(source, filename, true_year) |>
  gt() |>
  cols_label(
    n_true_years = "distinct_years"
  ) |>
